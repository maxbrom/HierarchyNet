function [currentLEO_index, bestLEO_position, predictedHandoverPoints, ping_LEO] = originalHandoverProcess(GEO_position, LEO_positions, LEO_velocities, user_position, currentLEO_index, c, network)
    % Move the non-Kalman implementation here along with its helper functions:
    % - collectLEOData
    % - createUserGraph
    % - UserDecisionMaking
    % - calculate_handover_weight_new
    % - getLEOSignalStrength
    % - coverage_overlap_new
    % - calculate_surface_distance
    % - etc.
    
    % The existing implementation remains the same, just rename from the if-else block
    ...
    fixed_delays = network.fixed.processing_delay + ...
                  network.fixed.equipment_delay + ...
                  network.fixed.ground_station_delay;
                  
    round_trip_base = 2 * (base_propagation + fixed_delays);
    ping_LEO = round_trip_base + ...
               network.variable.network_jitter + ...
               network.variable.packet_loss_delay + ...
               network.variable.final_variation;
